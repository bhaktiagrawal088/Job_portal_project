...
const CompanySetup = () => {
  ...
  // useEffect(() => {
  //   if (!singleCompany || singleCompany.id !== params.id) {
  //     // Fetch the single company data if not already available or wrong company
  //     dispatch(singleCompany(params.id));
  //   }
      ...
      <div className='max-w-xl mx-auto my-10'>
        ...
        <div className='flex items-center gap-5 p-8'>
          <Button onClick = { () =>  navigate('/admin/companies')}
           className="flex items-center gap-2 text-gray-600 font-semibold"><ArrowLeft/><span>Back</span></Button>
        ...
        {/* <Button type="submit" className="w-full mt-8 bg-black text-white hover:bg-black">Update</Button> */}
        {
                        loading ? <Button className="w-full my-4" ><Loader2 className='mr-2 h-4 w-4 animate-spin'/>Please wait</Button> :
                        <Button className="w-full mb-2 mt-8 text-white bg-black hover:bg-black">Update</Button>
        }
        </form>
      </div>
    </>
